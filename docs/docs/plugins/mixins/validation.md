...
!!! info "More Info"
    For more information on any of the methods described below, refer to the InvenTree source code. [A working example is available as a starting point]({{ sourcefile("src/backend/Tracklet/plugin/samples/integration/validation_sample.py") }}).
