...
def is_windows_environment() -> bool:
    """Return True when running on a native Windows host."""
    return os.name == 'nt'
...
def pip_cmd() -> str:
    """Return the pip command for the active Python environment."""
    return f'{python_cmd()} -m pip'


def supports_pty() -> bool:
    """Return True if the current platform supports pty-backed commands."""
    return not is_windows_environment()


def waitress_cmd(address: str) -> str:
    """Return a waitress command line for the provided bind address."""
    host, port = address.rsplit(':', 1)
    return (
        f'{python_cmd()} -m waitress.runner --host {host} --port {port} '
        f'Tracklet.wsgi:application'
    )
...
def envcheck_invoke_cmd():
    ...
    intendded = ['/bin/invoke', '/bin/inv', '\\Scripts\\invoke.exe', '\\Scripts\\inv.exe']

    correct_cmd: Optional[str] = None
    if (
        is_windows_environment()
        or is_rtd_environment()
        or is_docker_environment()
        or is_devcontainer_environment()
    ):
...
def run(c, cmd, path: Optional[Path] = None, pty=False, env=None):
    ...
    pty = pty and supports_pty()

    try:
        with c.cd(str(path)):
            c.run(cmd, pty=pty, env=env)
...
def manage(c, cmd, pty: bool = False, env=None):
    ...
    Args:
        ...
        env (dict, optional): Environment variables to pass to the command. Defaults to None.
    """
    run(c, f'{python_cmd()} manage.py {cmd}', manage_py_dir(), pty, env)
...
def run_install(
    ...
    if not uv:
        if run_preflight:
            run(
                c,
                f'{pip_cmd()} install --no-cache-dir --disable-pip-version-check -U pip setuptools',
            )
        run(
            c,
            f'{pip_cmd()} install --no-cache-dir --disable-pip-version-check -U {"--require-hashes" if pinned else ""} -r {install_file}',
        )
    else:
        if run_preflight:
            run(
                c,
                f'{pip_cmd()} install --no-cache-dir --disable-pip-version-check -U uv setuptools',
            )
...
@task(
    help={
        ...
        'windows': 'Use the Windows compatibility dependency set',
    }
)
@state_logger('TASK02')
def install(c, uv=False, skip_plugins=False, dev=False, windows=False):
    """Installs required python packages."""
    windows = windows or is_windows_environment()

    if windows:
        info('Using Windows compatibility dependency profile')
    ...
    if dev:
        if windows:
            run_install(
                c,
                uv,
                local_dir().joinpath('src/backend/requirements-windows.in'),
                version_check=False,
                pinned=False,
            )
            run_install(
                c,
                uv,
                local_dir().joinpath('src/backend/requirements-windows-dev.in'),
                run_preflight=False,
                version_check=False,
                pinned=False,
            )
    ...
    run_install(c, uv, install_file, version_check=not windows, pinned=not windows)
...
def setup_dev(c, tests=False):
    ...
    # Install required Python packages with PIP
    install(c, uv=False, skip_plugins=True, dev=True, windows=is_windows_environment())
...
def gunicorn(c, address='0.0.0.0:8000', workers=None):
    ...
    if is_windows_environment():
        cmd = waitress_cmd(address)
        info(f'Starting Waitress Server: {cmd}')
        run(c, cmd, path=manage_py_dir(), pty=True)
        return
