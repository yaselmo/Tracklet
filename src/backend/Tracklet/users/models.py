...
from Tracklet.ready import isImportingData
...
def default_token_expiry():
    ...
    return Tracklet.helpers.current_date() + datetime.timedelta(days=365)


class ApiToken(AuthToken, Tracklet.models.MetadataMixin):
    ...
    def sanitize_name(name: str) -> str:
        ...
        name = Tracklet.helpers.remove_non_printable_characters(
            name, remove_newline=True
        )
        name = Tracklet.helpers.strip_html_tags(name)
    ...
    def expired(self) -> bool:
        ...
        return (
            self.expiry is not None and self.expiry < Tracklet.helpers.current_date()
        )
...
class UserProfile(Tracklet.models.MetadataMixin):
