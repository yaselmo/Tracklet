...
class StockImageTestMixin:
    """Mixin for testing stock item images."""

    @staticmethod
    def create_upload_image(name='stock_image.png', color='green'):
        """Create an in-memory image upload payload."""
        buffer = io.BytesIO()
...
class StockItemTest(StockImageTestMixin, StockAPITestCase):
    ...
    def test_stock_item_create(self):
        ...
        self.assertIsNone(response.data[0]['part'])
        self.assertEqual(response.data[0]['name'], 'Independent Item')
        ...
        response = self.post(
            self.list_url, {'name': 'Needs Quantity', 'part': 1, 'location': 1}, expected_code=400
        )
    ...
    def test_stock_item_create_with_broken_and_missing_quantities(self):
        """Broken and missing quantities should be accepted when valid."""
        response = self.post(
            self.list_url,
            data={
                'name': 'Damaged Batch',
        ...
        self.assertEqual(created['broken_quantity'], 2)
        self.assertEqual(created['missing_quantity'], 1)
        self.assertEqual(created['available'], 7)

    def test_stock_item_rejects_invalid_broken_and_missing_quantities(self):
        """Broken and missing quantities must not exceed total stock."""
        response = self.post(
            self.list_url,
            data={
                'name': 'Invalid Damaged Batch',
                'location': 1,
                'quantity': 5,
                'broken_quantity': 3,
                'missing_quantity': 3,
        ...
        self.assertIn('broken_quantity', response.data)
        self.assertIn('missing_quantity', response.data)

    def test_stock_item_update_without_part(self):
        """A stock item without a linked part can still be edited."""
        item = StockItem.objects.create(
            name='Before Name',
        ...
        self.assertEqual(response.data['notes'], 'after')
        self.assertIsNone(response.data['part'])
        self.assertEqual(response.data['name'], 'After Name')
        self.assertEqual(response.data['category'], 'After Category')

        item.refresh_from_db()
        self.assertEqual(item.location_id, 2)
        self.assertEqual(item.notes, 'after')
    ...
    def test_stock_item_update_broken_and_missing_quantities(self):
        ...
        self.assertEqual(response.data['broken_quantity'], 2)
        self.assertEqual(response.data['missing_quantity'], 3)
        self.assertEqual(response.data['available'], 5)

        item.refresh_from_db()
        self.assertEqual(item.broken_quantity, 2)
        self.assertEqual(item.missing_quantity, 3)

        tracking = item.tracking_info.last()
        self.assertEqual(tracking.tracking_type, StockHistoryCode.EDITED.value)
        self.assertEqual(tracking.deltas['broken_quantity'], 2.0)
        self.assertEqual(tracking.deltas['missing_quantity'], 3.0)

    def test_stock_item_image_upload_and_remove(self):
        """A stock item image can be uploaded and removed via the detail endpoint."""
        item = StockItem.objects.create(
            part_id=1,
            location_id=1,
            quantity=1,
        ...
        upload = self.create_upload_image()
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn('/media/stock_images/', response.data['image'])
        self.assertIn('/media/stock_images/', response.data['thumbnail'])
    ...
    def test_stock_item_existing_image_reuse(self):
        """An already uploaded stock image can be reused without re-uploading."""
        first_item = StockItem.objects.create(
            part_id=1,
            location_id=1,
            quantity=1,
        )
        ...
        response = self.client.patch(
            reverse('api-stock-detail', kwargs={'pk': first_item.pk}),
            {'image': self.create_upload_image(name='reuse_me.png')},
            format='multipart',
        )
        ...
        response = self.patch(
            reverse('api-stock-detail', kwargs={'pk': second_item.pk}),
        ...
        self.assertEqual(str(first_item.image), str(second_item.image))

    def test_stock_item_available_excludes_broken_missing_and_allocated(self):
        """Available stock should exclude broken, missing, and allocated quantities."""
        assembly = part.models.Part.objects.create(
            name='API Broken Assembly',
            description='Assembly for API broken quantity test',
            assembly=True,
        )
        component = part.models.Part.objects.create(
            name='API Broken Component',
            description='Component for API broken quantity test',
            component=True,
        )
        part.models.BomItem.objects.create(part=assembly, sub_part=component, quantity=1)

        build = build.models.Build.objects.create(
            part=assembly,
            quantity=1,
            reference='BO-API-BROKEN',
        )
        build_line = build.build_lines.get(bom_item__sub_part=component)
        ...
        self.assertEqual(response.data['allocated'], 4)
        self.assertEqual(response.data['available'], 1)

    def test_stock_item_delete_without_part(self):
        """A stock item without a linked part can be deleted safely."""
        item = StockItem.objects.create(name='Delete Me', location_id=1, quantity=1)
    ...
    def test_stock_item_detail_prefers_owned_name_and_category(self):
        """Stock API should return the StockItem-owned name/category values."""
        item = StockItem.objects.create(
            name='Owned Name',
        ...
        self.assertEqual(response.data['name'], 'Owned Name')
        self.assertEqual(response.data['category'], 'Owned Category')
        self.assertEqual(response.data['part_detail']['name'], 'Widget')
