...
STOCK_ITEM_IMAGE_DIR = 'stock_images'
...
def get_stock_item_image_directory():
    """Return the absolute filesystem directory for stock item images."""
    return os.path.join(settings.MEDIA_ROOT, STOCK_ITEM_IMAGE_DIR)
...
class StockLocation(
    Tracklet.models.PluginValidationMixin,
    Tracklet.models.InvenTreeParameterMixin,
    Tracklet.models.InvenTreeBarcodeMixin,
    report.mixins.InvenTreeReportMixin,
    Tracklet.models.PathStringMixin,
    Tracklet.models.MetadataMixin,
    Tracklet.models.InvenTreeTree,
):
    ...
    def get_absolute_url(self):
        ...
        return Tracklet.helpers.pui_url(f'/stock/location/{self.id}')
...
class StockItem(
    ...
    Tracklet.models.InvenTreeImageMixin,
    Tracklet.models.InvenTreeBarcodeMixin,
    ...
    IMAGE_RENAME = rename_stock_item_image
    ...
    def report_context(self) -> StockItemReportContext:
        ...
        part = self.part if self.part_id else None
        item_name = self.name or (part.full_name if part else str(self))
        ...
        return {
            ...
            'parameters': part.parameters_map() if part else {},
            'quantity': Tracklet.helpers.normalize(self.quantity),
            ...
            'test_template_list': part.getTestTemplates() if part else [],
            'test_templates': part.getTestTemplateMap() if part else {},
        }
    ...
    EXPIRED_FILTER = (
        IN_STOCK_FILTER
        & ~Q(expiry_date=None)
        & Q(expiry_date__lt=Tracklet.helpers.current_date())
    )
    ...
    def _create_serial_numbers(cls, serials: list, **kwargs) -> QuerySet:
        ...
        if item_ids:
            items = StockItem.objects.filter(pk__in=item_ids)
        elif part is not None:
            items = StockItem.objects.filter(part=part, serial__in=serials)
        else:
            items = StockItem.objects.filter(serial__in=serials, part=None)
    ...
    def convert_serial_to_int(serial: str) -> int | None:
        ...
        if serial not in [None, '']:
            return Tracklet.helpers.extract_int(serial)
        else:
            return None
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            try:
                ...
                if old.broken_quantity != self.broken_quantity:
                    deltas['broken_quantity'] = float(self.broken_quantity)
                    deltas['old_broken_quantity'] = float(old.broken_quantity)

                if old.missing_quantity != self.missing_quantity:
                    deltas['missing_quantity'] = float(self.missing_quantity)
                    deltas['old_missing_quantity'] = float(old.missing_quantity)
    ...
    def validate_unique(self, exclude=None):
        ...
        if self.serial and self.part_id:
    ...
    def clean(self):
        ...
        try:
            ...
            if self.part and self.part.trackable and self.quantity != int(self.quantity):
                raise ValidationError({
            ...
            if (
                self.part
                and self.part.trackable
                and self.broken_quantity != int(self.broken_quantity)
            ):
                raise ValidationError({
                    'broken_quantity': _(
                        'Broken quantity must be integer value for trackable parts'
                    )
                })
            ...
            if (
                self.part
                and self.part.trackable
                and self.missing_quantity != int(self.missing_quantity)
            ):
                raise ValidationError({
                    'missing_quantity': _(
                        'Missing quantity must be integer value for trackable parts'
                    )
                })
        ...
        if (self.broken_quantity + self.missing_quantity) > self.quantity:
            raise ValidationError({
                'broken_quantity': _(
                    'Broken and missing quantities cannot exceed total stock quantity'
                ),
                'missing_quantity': _(
                    'Broken and missing quantities cannot exceed total stock quantity'
                ),
        ...
        try:
            if self.supplier_part is not None and self.part is not None:
        ...
        if self.build:
            if self.part is None:
                raise ValidationError({
                    'part': _('Build-backed stock items must reference a part')
                })
    ...
    def get_absolute_url(self):
        """Return url for instance."""
        return Tracklet.helpers.pui_url(f'/stock/item/{self.id}')
    ...
    def get_part_name(self):
        ...
        return _('Unassigned Stock Item')
    ...
    broken_quantity = models.DecimalField(
        verbose_name=_('Broken Quantity'),
        max_digits=15,
        decimal_places=5,
        validators=[MinValueValidator(0)],
        default=0,
        help_text=_('Quantity that is damaged and unusable'),
    )

    missing_quantity = models.DecimalField(
        verbose_name=_('Missing Quantity'),
        max_digits=15,
        decimal_places=5,
        validators=[MinValueValidator(0)],
        default=0,
        help_text=_('Quantity that is missing or unavailable'),
    )
    ...
    def is_stale(self):
        ...
        today = Tracklet.helpers.current_date()
    ...
    def usable_quantity(self):
        """Return the usable stock quantity after broken and missing amounts are excluded."""
        return max(
            self.quantity - self.broken_quantity - self.missing_quantity,
            Decimal(0),
        )

    def unallocated_quantity(self):
        """Return the quantity of this StockItem which is *not* allocated."""
        return max(self.usable_quantity() - self.allocation_count(), Decimal(0))
    ...
    def add_tracking_entry(
        ...
        **kwargs,
        ...
        entry = StockItemTracking(
            ...
            user=user,
            date=Tracklet.helpers.current_time(),
            notes=notes,
            deltas=deltas,
        )
    ...
    def merge_stock_items(self, other_items, raise_error=False, **kwargs):
        ...
        *This* stock item subsumes the other, which is essentially deleted:
        ...
        if not rebuild_result:
            ...
            Tracklet.tasks.offload_task(stock.tasks.rebuild_stock_items, group='stock')
    ...
    def stocktake(self, count, user, **kwargs):
        ...
        if self.updateQuantity(count):
            ...
            self.stocktake_date = Tracklet.helpers.current_date()
    ...
    def __str__(self):
        ...
        base_name = self.name or (
            self.part.full_name if self.part_id else str(_('Stock Item'))
        )

        if self.serial:
            s = f'{base_name} #{self.serial}'
        else:
            s = (
                f'{Tracklet.helpers.decimal2string(self.quantity)} x '
                f'{base_name}'
            )
    ...
    def getTestResults(self, template=None, test=None, result=None, user=None):
        ...
        if test:
            # Filter by test name
            test_key = Tracklet.helpers.generateTestKey(test)
    ...
    def requiredTestStatus(self, required_tests=None):
        ...
        for test in required_tests:
            key = Tracklet.helpers.generateTestKey(test.test_name)
...
def after_delete_stock_item(sender, instance: StockItem, **kwargs):
    ...
    if Tracklet.ready.isImportingData():
        return

    if instance.part_id and Tracklet.ready.canAppAccessDatabase(allow_test=True):
        # Run this check in the background
        Tracklet.tasks.offload_task(
...
def after_save_stock_item(sender, instance: StockItem, created, **kwargs):
    ...
    if not Tracklet.ready.isImportingData():
        if instance.part_id and Tracklet.ready.canAppAccessDatabase(allow_test=True):
            Tracklet.tasks.offload_task(
                part_tasks.notify_low_stock_if_required,
                instance.part.pk,
...
class StockItemTracking(Tracklet.models.InvenTreeModel):
    ...
    def get_absolute_url(self):
        """Return url for instance."""
        return Tracklet.helpers.pui_url(f'/stock/item/{self.item.id}')
...
class StockItemTestResult(Tracklet.models.InvenTreeMetadataModel):
    ...
    def key(self):
        """Return key for test."""
        return Tracklet.helpers.generateTestKey(self.test_name)
