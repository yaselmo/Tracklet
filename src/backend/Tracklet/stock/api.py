...
from django.db.models import Count, DecimalField, Exists, ExpressionWrapper, F, OuterRef, Q, Subquery, Sum, Value
...
from Tracklet.fields import InvenTreeOutputOption, OutputConfiguration
from Tracklet.filters import (
    ORDER_FILTER_ALIAS,
    SEARCH_ORDER_FILTER,
...
class StockThumbs(ListAPI):
    """API endpoint for retrieving reusable stock item images."""

    queryset = StockItem.objects.all()
    serializer_class = StockSerializers.StockThumbSerializer

    def get_queryset(self):
        """Return stock items which have an associated image."""
        ...
        if search:
            queryset = queryset.filter(
                ...
                | Q(part__description__icontains=search)
    ...
    def list(self, request, *args, **kwargs):
        """Serialize available stock item images."""
        queryset = self.get_queryset()

        data = (
            queryset.values('image')
            .annotate(count=Count('image'))
            .order_by('-count', 'image')
        )
...
class StockFilter(FilterSet):
    ...
    # Stock item name filters
    name = rest_filters.CharFilter(
        label=_('Stock item name (case insensitive)'),
        field_name='name',
        lookup_expr='iexact',
    )

    name_contains = rest_filters.CharFilter(
        label=_('Stock item name contains (case insensitive)'),
        field_name='name',
        lookup_expr='icontains',
    )

    name_regex = rest_filters.CharFilter(
        label=_('Stock item name (regex)'), field_name='name', lookup_expr='iregex'
    )
    ...
    def filter_available(self, queryset, name, value):
        ...
        Here, "available" means that the allocated quantity is less than the usable
        stock quantity after broken and missing units are excluded.
        """
        ...
        if project_id:
            ...
            if project is not None:
                ...
                overlapping_allocation_sq = (
                    ProjectStockAllocation.objects.filter(stock_item=OuterRef('pk'))
                    .filter(
                        overlapping_project_reservation_filter(project, prefix='project__')
                    )
                    .values('stock_item')
                ...
                queryset = queryset.annotate(
                    overlapping_project_allocated=Coalesce(
                        Subquery(overlapping_allocation_sq, output_field=quantity_field),
                        zero,
                        output_field=quantity_field,
                    ),
                    overlapping_project_instrument_allocated=Coalesce(
                        Subquery(overlapping_instrument_sq, output_field=quantity_field),
                        zero,
                        output_field=quantity_field,
                    ),
                    has_overlapping_project_instrument=Exists(
                        ProjectInstrument.objects.filter(stock_item=OuterRef('pk')).filter(
                            overlapping_project_reservation_filter(
                                project, prefix='project__'
                            )
                        )
                    ),
                    already_in_project=Exists(
                    ...
                    project_context_allocated=ExpressionWrapper(
                        F('sales_allocated')
                        + F('build_allocated')
                        + Greatest(
                            F('overlapping_project_allocated'),
                            F('overlapping_project_instrument_allocated'),
                            output_field=quantity_field,
                        ),
                        output_field=quantity_field,
                    )
                ...
                project_available_filter = (
                    Q(StockItem.IN_STOCK_FILTER)
                    & Q(tracklet_status__in=['IN_STOCK', 'IN_USE'])
                    & (
                        Q(usable_quantity__gt=F('project_context_allocated'))
                        | Q(already_in_project=True)
                    )
                )

                if str2bool(value):
                    return queryset.filter(project_available_filter)

                return queryset.exclude(project_available_filter)

        if str2bool(value):
            # The usable quantity field is greater than the calculated allocated field
            # Note that the item must also be "in stock" and available in Tracklet lifecycle.
            return queryset.filter(
                StockItem.IN_STOCK_FILTER,
                tracklet_status__in=['IN_STOCK', 'IN_USE'],
                usable_quantity__gt=F('allocated'),
            )
        # The usable quantity field is less than (or equal to) the calculated allocated field
        return queryset.exclude(
            StockItem.IN_STOCK_FILTER
            & Q(tracklet_status__in=['IN_STOCK', 'IN_USE'])
            & Q(usable_quantity__gt=F('allocated'))
        )

    broken = rest_filters.BooleanFilter(label='Broken', method='filter_broken')

    def filter_broken(self, queryset, name, value):
        """Filter by whether the stock item has any broken quantity."""
        if str2bool(value):
            return queryset.filter(broken_quantity__gt=0)
        return queryset.filter(broken_quantity__lte=0)

    missing = rest_filters.BooleanFilter(label='Missing', method='filter_missing')

    def filter_missing(self, queryset, name, value):
        """Filter by whether the stock item has any missing quantity."""
        if str2bool(value):
            return queryset.filter(missing_quantity__gt=0)
        return queryset.filter(missing_quantity__lte=0)
...
class StockList(
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not data.get('name'):
            if part:
                data['name'] = part.full_name
            else:
                raise ValidationError({'name': _('Stock item name is required')})

        if not data.get('category') and part and part.category_id:
            data['category'] = part.category.pathstring

        location = data.get('location', None)
        # Override location if not specified
        if location is None and part and part.default_location:
            data['location'] = part.default_location.pk

        expiry_date = data.get('expiry_date', None)

        # An expiry date was *not* specified - try to infer it!
        if expiry_date is None and part and part.default_expiry > 0:
            data['expiry_date'] = Tracklet.helpers.current_date() + timedelta(
        ...
        if serial_numbers:
            if not part:
                try:
                    serials = extract_serial_numbers(serial_numbers, quantity, None)
                except DjangoValidationError as e:
                    raise ValidationError({
                        'quantity': e.messages,
                        'serial_numbers': e.messages,
                    })
            elif not part.trackable:
                raise ValidationError({
                    'serial_numbers': [
    ...
    ordering = ['name', 'quantity', 'location']
...
stock_api_urls = [
    ...
    path('thumbs/', StockThumbs.as_view(), name='api-stock-thumbs-list'),
