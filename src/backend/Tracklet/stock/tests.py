...
from Tracklet.unit_test import AdminTestCase, InvenTreeTestCase
...
class StockTest(StockTestBase):
    ...
    def test_broken_missing_validation(self):
        """Broken and missing quantities must stay within valid stock bounds."""
        item = StockItem(
            part=Part.objects.get(pk=1),
        ...
        item.broken_quantity = -1

        with self.assertRaises(ValidationError):
            item.full_clean()

        item.broken_quantity = 8
        item.missing_quantity = 4

        with self.assertRaises(ValidationError):
            item.full_clean()

    def test_unallocated_quantity_excludes_broken_and_missing(self):
        """Unallocated quantity should exclude broken, missing, and allocated stock."""
        assembly = Part.objects.create(
            name='Broken Quantity Assembly',
            description='Assembly for broken quantity test',
            assembly=True,
        )
        component = Part.objects.create(
            name='Broken Quantity Component',
            description='Component for broken quantity test',
            component=True,
        )
        ...
        build = Build.objects.create(
            reference='BO-BROKEN-1',
            part=assembly,
            title='Broken quantity test build',
            quantity=1,
        )
        ...
        self.assertEqual(item.usable_quantity(), 5)
        self.assertEqual(item.unallocated_quantity(), 1)
