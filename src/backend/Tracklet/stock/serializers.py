...
class LocationBriefSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class StockItemTestResultSerializer(
    ...
    Tracklet.serializers.InvenTreeModelSerializer,
):
    ...
    attachment = Tracklet.serializers.InvenTreeAttachmentSerializerField(
        required=False,
        allow_null=True,
        label=_('Attachment'),
    ...
    def validate(self, data):
        ...
        if not template:
            ...
            test_key = Tracklet.helpers.generateTestKey(test_name)
...
class StockItemSerializer(
    ...
    Tracklet.serializers.RemoteImageMixin,
    Tracklet.serializers.InvenTreeTagModelSerializer,
):
    ...
    import_exclude_fields = ['location_path', 'serial_numbers', 'use_pack_size', 'image']
    ...
    name = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text=_('Name for this stock item'),
        label=_('Name'),
    )
    ...
    category = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text=_('Category for this stock item'),
        label=_('Category'),
    )
    ...
    image = Tracklet.serializers.InvenTreeImageSerializerField(
    ...
    existing_image = serializers.CharField(
        ...
        help_text=_('Filename of an existing stock item image'),
        write_only=True,
        required=False,
        allow_blank=False,
    )
    ...
    def validate_existing_image(self, img):
        """Validate the selected reusable image file."""
        if not img:
            return img

        normalized = str(img).replace('\\', '/').split('/')[-1]
        img_path = os.path.join(settings.MEDIA_ROOT, 'stock_images', normalized)
    ...
    def validate_name(self, value):
        """Ensure stock items always have their own editable name."""
        value = (value or '').strip()

        return value

    def validate(self, data):
        """Validate broken and missing quantities against the total stock quantity."""
        data = super().validate(data)
        ...
        quantity = data.get(
            'quantity',
            instance.quantity if instance is not None else Decimal(0),
        )
        broken_quantity = data.get(
            'broken_quantity',
            instance.broken_quantity if instance is not None else Decimal(0),
        )
        missing_quantity = data.get(
            'missing_quantity',
            instance.missing_quantity if instance is not None else Decimal(0),
        )

        errors = {}

        if broken_quantity < 0:
            errors['broken_quantity'] = _(
                'Broken quantity must be greater than or equal to zero'
            )

        if missing_quantity < 0:
            errors['missing_quantity'] = _(
                'Missing quantity must be greater than or equal to zero'
            )

        if (broken_quantity + missing_quantity) > quantity:
            message = _('Broken and missing quantities cannot exceed total stock quantity')
            errors['broken_quantity'] = message
            errors['missing_quantity'] = message
    ...
    def save(self):
        """Save the StockItem instance and attach any downloaded remote image."""
        super().save()

        stock_item = self.instance
        data = self.validated_data
        existing_image = data.pop('existing_image', None)

        if existing_image and stock_item:
            stock_item.image = os.path.join('stock_images', existing_image)
            stock_item.save()

        remote_img = getattr(self, 'remote_image_file', None)

        if remote_img and stock_item:
            fmt = remote_img.format or 'PNG'
            buffer = io.BytesIO()
            remote_img.save(buffer, format=fmt)

            filename = f'stock_item_{stock_item.pk}_image.{fmt.lower()}'

            stock_item.image.save(filename, ContentFile(buffer.getvalue()))
    ...
    def annotate_queryset(queryset):
        ...
        queryset = queryset.annotate(
            usable_quantity=Greatest(
                ExpressionWrapper(
                    F('quantity') - F('broken_quantity') - F('missing_quantity'),
                    output_field=DecimalField(max_digits=20, decimal_places=6),
                ),
                Value(Decimal(0)),
                output_field=DecimalField(max_digits=20, decimal_places=6),
        ...
        queryset = queryset.annotate(
            available=ExpressionWrapper(
                Case(
                    When(
                        tracklet_status__in=[
                            TrackletStockStatus.IN_STOCK,
                            TrackletStockStatus.IN_USE,
                        ],
                        then=Greatest(
                            ExpressionWrapper(
                                F('usable_quantity') - F('allocated'),
        ...
        stale_date = Tracklet.helpers.current_date() + timedelta(days=stale_days)
        stale_filter = (
    ...
    broken_quantity = InvenTreeDecimalField(required=False)
    missing_quantity = InvenTreeDecimalField(required=False)
    ...
    purchase_price = Tracklet.serializers.InvenTreeMoneySerializer(
...
class StockThumbSerializer(serializers.Serializer):
    """Serializer for existing stock item images."""
...
class SerializeStockItemSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        try:
            serials = Tracklet.helpers.extract_serial_numbers(
                serial_numbers,
                quantity,
...
class StockStatusCustomSerializer(serializers.ChoiceField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if Tracklet.ready.isGeneratingSchema():
            kwargs['help_text'] = (
                kwargs['help_text']
                + '\n\n'
                + '\n'.join(
...
class StockChangeTrackletStatusSerializer(serializers.Serializer):
    """Serializer for changing stock condition values of multiple StockItem objects."""

    class Meta:
        """Metaclass options."""

        fields = ['items', 'tracklet_status', 'broken_quantity', 'missing_quantity', 'note']
    ...
    broken_quantity = InvenTreeDecimalField(
        required=False,
        label=_('Broken Quantity'),
        help_text=_('Damaged and unusable quantity'),
    )

    missing_quantity = InvenTreeDecimalField(
        required=False,
        label=_('Missing Quantity'),
        help_text=_('Lost or unavailable quantity'),
    )
    ...
    def validate(self, data):
        """Validate quantity constraints for the selected items."""
        broken_quantity = data.get('broken_quantity', None)
        missing_quantity = data.get('missing_quantity', None)

        if broken_quantity is not None and broken_quantity < 0:
            raise ValidationError({'broken_quantity': _('Broken quantity cannot be negative')})

        if missing_quantity is not None and missing_quantity < 0:
            raise ValidationError({'missing_quantity': _('Missing quantity cannot be negative')})

        for item in data.get('items', []):
            broken = broken_quantity if broken_quantity is not None else item.broken_quantity
            missing = missing_quantity if missing_quantity is not None else item.missing_quantity

            if (broken + missing) > item.quantity:
                message = _('Broken quantity plus missing quantity cannot exceed stock quantity')
                raise ValidationError(
                    {
                        'broken_quantity': message,
    ...
    @transaction.atomic
    def save(self):
        """Save the serializer to change stock condition values for selected items."""
        data = self.validated_data

        items = data['items']
        tracklet_status = data.get('tracklet_status', None)
        broken_quantity = data.get('broken_quantity', None)
        missing_quantity = data.get('missing_quantity', None)
        ...
        for item in items:
            ...
            if tracklet_status is not None and item.tracklet_status != tracklet_status:
                deltas['tracklet_status'] = tracklet_status
                deltas['old_tracklet_status'] = item.tracklet_status

            if broken_quantity is not None and item.broken_quantity != broken_quantity:
                deltas['broken_quantity'] = float(broken_quantity)
                deltas['old_broken_quantity'] = float(item.broken_quantity)

            if missing_quantity is not None and item.missing_quantity != missing_quantity:
                deltas['missing_quantity'] = float(missing_quantity)
                deltas['old_missing_quantity'] = float(item.missing_quantity)
            ...
            if tracklet_status is not None:
                update_data['tracklet_status'] = tracklet_status

            if broken_quantity is not None:
                update_data['broken_quantity'] = broken_quantity

            if missing_quantity is not None:
                update_data['missing_quantity'] = missing_quantity

            # Update directly to avoid model-level auto-sync recalculating
            # the just-selected status during this manual override action.
            StockItem.objects.filter(pk=item.pk).update(**update_data)
            if tracklet_status is not None:
                item.tracklet_status = tracklet_status
            if broken_quantity is not None:
                item.broken_quantity = broken_quantity
            if missing_quantity is not None:
                item.missing_quantity = missing_quantity
...
class LocationTreeSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class StockItemSerialNumbersSerializer(Tracklet.serializers.InvenTreeModelSerializer):
