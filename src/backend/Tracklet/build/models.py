...
class Build(
    ...
    OVERDUE_FILTER = (
        ...
        & ~Q(target_date=None)
        & Q(target_date__lte=Tracklet.helpers.current_date())
    )
    ...
    def get_absolute_url(self):
        ...
        return Tracklet.helpers.pui_url(f'/manufacturing/build-order/{self.id}')
    ...
    def getNextBuildNumber(cls):
        ...
        while 1:
            new_ref = Tracklet.helpers.increment(new_ref)
    ...
    def _action_complete(self, *args, **kwargs):
        ...
        self.completion_date = Tracklet.helpers.current_date()
        self.completed_by = user
        self.status = BuildStatus.COMPLETE.value
        self.save()

        # Offload task to complete build allocations
        if not Tracklet.tasks.offload_task(
        ...
        context = {
            ...
            'link': Tracklet.helpers_model.construct_absolute_url(
    ...
    def _action_cancel(self, *args, **kwargs):
        ...
        Tracklet.helpers_model.notify_responsible(
            self,
            Build,
...
def after_save_build(sender, instance: Build, created: bool, **kwargs):
    ...
    if Tracklet.ready.isImportingData() or not Tracklet.ready.canAppAccessDatabase(
        allow_test=True
    ):
        return
...
class BuildLine(report.mixins.InvenTreeReportMixin, Tracklet.models.InvenTreeModel):
...
class BuildItem(Tracklet.models.InvenTreeMetadataModel):
    ...
    def check_allocated_quantity(self, raise_error: bool = False):
        ...
        if self.quantity > self.stock_item.quantity:
            q = Tracklet.helpers.normalize(self.quantity)
            a = Tracklet.helpers.normalize(self.stock_item.quantity)
