...
from Tracklet.ready import isGeneratingSchema
...
class CategorySerializer(
    ...
    def validate(self, attrs):
        """Ensure calibration interval is provided when calibration is enabled."""
        attrs = super().validate(attrs)

        requires_calibration = attrs.get(
            'requires_calibration',
            getattr(self.instance, 'requires_calibration', False),
        )
        calibration_interval_days = attrs.get(
            'calibration_interval_days',
            getattr(self.instance, 'calibration_interval_days', None),
        )

        if requires_calibration and not calibration_interval_days:
            raise serializers.ValidationError({
                'calibration_interval_days': _(
                    'Calibration interval is required when calibration is enabled'
                )
            })
...
class CategoryTree(Tracklet.serializers.InvenTreeModelSerializer):
...
class PartTestTemplateSerializer(
    DataImportExportSerializerMixin, Tracklet.serializers.InvenTreeModelSerializer
):
...
class PartSalePriceSerializer(
    ...
    quantity = Tracklet.serializers.InvenTreeDecimalField()

    price = Tracklet.serializers.InvenTreeMoneySerializer(allow_null=True)

    price_currency = Tracklet.serializers.InvenTreeCurrencySerializer(
        help_text=_('Purchase currency of this stock item')
    )


class PartInternalPriceSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class PartThumbSerializer(serializers.Serializer):
    ...
    image = Tracklet.serializers.InvenTreeImageSerializerField(read_only=True)
    count = serializers.IntegerField(read_only=True)


class PartThumbSerializerUpdate(Tracklet.serializers.InvenTreeModelSerializer):
    ...
    def validate_image(self, value):
        ...
        validate = Tracklet.helpers.TestIfImage(value)
        if not validate:
            raise serializers.ValidationError(_('File is not an image'))
        return value

    image = Tracklet.serializers.InvenTreeAttachmentSerializerField(required=True)
...
class PartBriefSerializer(
    ...
    category = serializers.CharField(source='category.pathstring', read_only=True)
    ...
    pricing_min = enable_filter(
        Tracklet.serializers.InvenTreeMoneySerializer(
...
class DefaultLocationSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class PartSerializer(
    Tracklet.serializers.FilterableSerializerMixin,
    DataImportExportSerializerMixin,
    Tracklet.serializers.NotesFieldMixin,
    Tracklet.serializers.RemoteImageMixin,
    Tracklet.serializers.InvenTreeTaggitSerializer,
    Tracklet.serializers.InvenTreeModelSerializer,
):
...
class PartBomValidateSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class PartRequirementsSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class PartStocktakeSerializer(
    ...
    cost_min = Tracklet.serializers.InvenTreeMoneySerializer(allow_null=True)
    cost_min_currency = Tracklet.serializers.InvenTreeCurrencySerializer()

    cost_max = Tracklet.serializers.InvenTreeMoneySerializer(allow_null=True)
    cost_max_currency = Tracklet.serializers.InvenTreeCurrencySerializer()
...
class PartPricingSerializer(Tracklet.serializers.InvenTreeModelSerializer):
    ...
    # Custom serializers
    bom_cost_min = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )
    bom_cost_max = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )

    purchase_cost_min = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )
    purchase_cost_max = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )

    internal_cost_min = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )
    internal_cost_max = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )

    supplier_price_min = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )
    supplier_price_max = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )

    variant_cost_min = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )
    variant_cost_max = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )

    override_min = Tracklet.serializers.InvenTreeMoneySerializer(
    ...
    override_max = Tracklet.serializers.InvenTreeMoneySerializer(
    ...
    overall_min = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )
    overall_max = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )

    sale_price_min = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )
    sale_price_max = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )

    sale_history_min = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )
    sale_history_max = Tracklet.serializers.InvenTreeMoneySerializer(
        allow_null=True, read_only=True
    )
...
class PartSerialNumberSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class PartRelationSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class PartStarSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class BomItemSubstituteSerializer(Tracklet.serializers.InvenTreeModelSerializer):
...
class BomItemSerializer(
    ...
    quantity = Tracklet.serializers.InvenTreeDecimalField(required=True)

    setup_quantity = Tracklet.serializers.InvenTreeDecimalField(required=False)

    attrition = Tracklet.serializers.InvenTreeDecimalField(required=False)

    rounding_multiple = Tracklet.serializers.InvenTreeDecimalField(
        required=False, allow_null=True
    )
    ...
    pricing_min_total = enable_filter(
        Tracklet.serializers.InvenTreeMoneySerializer(allow_null=True, read_only=True),
        True,
        filter_name='pricing',
    )
