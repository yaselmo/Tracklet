...
from Tracklet.config import get_testfolder_dir
...
class PartCategoryAPITest(InvenTreeAPITestCase):
    ...
    def test_category_calibration_fields(self):
        """Part categories support calibration configuration."""
        url = reverse('api-part-category-list')
        ...
        self.assertEqual(response.data['requires_calibration'], True)
        self.assertEqual(response.data['calibration_interval_days'], 180)

    def test_category_calibration_requires_interval(self):
...
class PartOptionsAPITest(InvenTreeAPITestCase):
    ...
    def test_category(self):
        ...
        self.assertIn('requires_calibration', actions)
        self.assertIn('calibration_interval_days', actions)
...
class PartAPITest(PartAPITestBase):
    ...
    def test_cat_detail(self):
        ...
        data['requires_calibration'] = True
        data['calibration_interval_days'] = 90
        response = self.patch(url, data)
        self.assertEqual(response.data['description'], 'Changing the description')
        self.assertIsNone(response.data['parent'])
        self.assertEqual(response.data['requires_calibration'], True)
        self.assertEqual(response.data['calibration_interval_days'], 90)
