...
import Tracklet.cache
import Tracklet.ready
from common.settings import get_global_setting, set_global_setting
from Tracklet.config import get_plugin_dir
...
class PluginsRegistry:
    ...
    def set_ready(self):
        ...
        if InvenTreeSetting.get_setting('PLUGIN_ON_STARTUP', create=False, cache=False):
            if Tracklet.ready.isInTestMode() and not settings.PLUGIN_TESTING:
                # Ignore plugin reload in test mode
                pass
            elif Tracklet.ready.isRunningBackup():
                # Ignore plugin reload during backup
                pass
            elif Tracklet.ready.isGeneratingSchema():
                # Ignore plugin reload during schema generation
                pass
            elif Tracklet.ready.isInWorkerThread() or Tracklet.ready.isInMainThread():
    ...
    def with_mixin(
        ...
        configs = Tracklet.cache.get_session_cache(cache_key)
        ...
        if not configs:
            try:
                ...
                Tracklet.cache.set_session_cache(cache_key, configs)
    ...
    def _load_plugins(
        ...
        if Tracklet.ready.canAppAccessDatabase():
    ...
    def _update_urls(self):
        ...
        """
        from Tracklet.urls import urlpatterns
    ...
    def check_reload(self):
        ...
        if Tracklet.cache.get_session_cache('plugin_registry_checked'):
            # Return early if the registry has already been checked (for this request)
            return False

        Tracklet.cache.set_session_cache('plugin_registry_checked', True)
