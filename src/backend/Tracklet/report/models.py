...
from Tracklet.helpers_model import get_base_url
from Tracklet.models import MetadataMixin
...
def log_report_error(*args, **kwargs):
    ...
    if do_log:
        Tracklet.exceptions.log_error(*args, **kwargs)
...
class ReportTemplateBase(MetadataMixin, Tracklet.models.InvenTreeModel):
    ...
    def render(self, instance, request=None, context=None, **kwargs) -> bytes:
        ...
        if HTML is None:
            raise ValidationError(
                _(
                    'PDF rendering is unavailable because the optional weasyprint dependency is not installed for this host'
                )
    ...
    def base_context(self, request=None) -> BaseContextExtension:
        ...
        return {
            ...
            'date': Tracklet.helpers.current_date(),
            'datetime': Tracklet.helpers.current_time(),
            'template': self,
    ...
    def get_plugin_context(self, instance, request, context):
        ...
        for plugin in registry.with_mixin(PluginMixinEnum.REPORT):
            try:
                ...
                Tracklet.exceptions.log_error('add_report_context', plugin=plugin.slug)
    ...
    def notify_plugins(self, instance, report, request):
        ...
        for plugin in report_plugins:
            try:
                ...
                Tracklet.exceptions.log_error('report_callback', plugin=plugin.slug)
    ...
    def get_context(self, instance, request=None, **kwargs):
        ...
        for plugin in plugins:
            ...
            try:
                ...
                Tracklet.exceptions.log_error('add_label_context', plugin=plugin.slug)
    ...
    def print(
        ...
        **kwargs,
        ...
        try:
            ...
            output.delete()
            Tracklet.exceptions.log_error('print_labels', plugin=plugin.slug)
