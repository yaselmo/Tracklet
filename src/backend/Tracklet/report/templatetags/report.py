...
def uploaded_image(
    ...
    **kwargs,
    ...
    if exists and validate and not Tracklet.helpers.TestIfImage(full_path):
...
def part_image(part: Part, preview: bool = False, thumbnail: bool = False, **kwargs):
    ...
    return uploaded_image(
        Tracklet.helpers.image2name(part.image, preview, thumbnail), **kwargs
    )
...
def company_image(
    ...
    return uploaded_image(
        Tracklet.helpers.image2name(company.image, preview, thumbnail), **kwargs
    )
...
def logo_image(**kwargs) -> str:
    ...
    return Tracklet.helpers.getLogoImage(as_file=not debug_mode, **kwargs)
...
def internal_link(link, text) -> str:
    ...
    Uses the Tracklet.helpers_model.construct_absolute_url function to build the URL.
    """
    text = str(text)

    try:
        url = Tracklet.helpers_model.construct_absolute_url(link)
    except Exception:
        url = None
...
def render_currency(money, **kwargs):
    ...
    return Tracklet.helpers_model.render_currency(money, **kwargs)
...
def format_datetime(
    ...
    dt = Tracklet.helpers.to_local_time(dt, timezone)
...
def format_date(dt: date, timezone: Optional[str] = None, fmt: Optional[str] = None):
    ...
    try:
        dt = Tracklet.helpers.to_local_time(dt, timezone).date()
    except TypeError:
        return str(dt)
...
@font-face {'{'}
    ...
    src: url('{Tracklet.helpers_model.construct_absolute_url(url)}') format('{font_format}');
{'}'}\n""")
