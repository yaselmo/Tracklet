...
from Tracklet.fields import InvenTreeURLField, RoundingDecimalField
...
class Company(
    ...
    Tracklet.models.InvenTreeMetadataModel,
):
    ...
    def get_absolute_url(self):
        ...
        return Tracklet.helpers.pui_url(f'/purchasing/manufacturer/{self.id}')
...
class Contact(Tracklet.models.InvenTreeMetadataModel):
...
class Address(Tracklet.models.InvenTreeModel):
...
class SupplierPart(
    ...
    def get_absolute_url(self):
        ...
        return Tracklet.helpers.pui_url(f'/purchasing/supplier-part/{self.id}')
    ...
    def clean(self):
        ...
        if self.pack_quantity and self.part:
            try:
                ...
                native_value = Tracklet.conversion.convert_physical_value(
                    self.pack_quantity, self.part.units, strip_units=False
                )

                # If part units are not provided, value must be dimensionless
                if not self.part.units and not Tracklet.conversion.is_dimensionless(
                    native_value
                ):
    ...
    def update_available_quantity(self, quantity):
        ...
        self.availability_updated = Tracklet.helpers.current_time()
        self.save()
