...
from Tracklet.fields import (
    InvenTreeModelMoneyField,
    InvenTreeURLField,
    RoundingDecimalField,
)
from Tracklet.helpers import decimal2string, pui_url
from Tracklet.helpers_model import notify_responsible
...
class TotalPriceMixin(models.Model):
    ...
    order_currency = models.CharField(
        ...
        validators=[Tracklet.validators.validate_currency_code],
    )
...
class Order(
    ...
    Tracklet.models.ReferenceIndexingMixin,
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.creation_date:
            self.creation_date = Tracklet.helpers.current_date()
...
class PurchaseOrder(TotalPriceMixin, Order):
    ...
    def _action_place(self, *args, **kwargs):
        ...
        if self.can_issue:
            ...
            self.issue_date = Tracklet.helpers.current_date()
            self.save()
    ...
    def _action_complete(self, *args, **kwargs):
        ...
        if self.status == PurchaseOrderStatus.PLACED:
            ...
            self.complete_date = Tracklet.helpers.current_date()
    ...
    def receive_line_items(
        ...
        for item in items:
            ...
            try:
                ...
                quantity = Tracklet.helpers.clean_decimal(quantity)
            except TypeError:
                raise ValidationError({'quantity': _('Invalid quantity provided')})
...
class SalesOrder(TotalPriceMixin, Order):
    ...
    def _action_complete(self, *args, **kwargs):
        ...
        bypass_shipped = Tracklet.helpers.str2bool(
        ...
        if self.shipment_date is None:
            ...
            self.shipment_date = Tracklet.helpers.current_date()
...
def after_save_sales_order(sender, instance: SalesOrder, created: bool, **kwargs):
    ...
    """
    if (
        not Tracklet.ready.canAppAccessDatabase(allow_test=True)
        or Tracklet.ready.isImportingData()
    ):
        return
...
class OrderLineItem(Tracklet.models.InvenTreeMetadataModel):
...
class PurchaseOrderLineItem(OrderLineItem):
    ...
    OVERDUE_FILTER = (
        ...
        & ~Q(target_date=None)
        & Q(target_date__lt=Tracklet.helpers.current_date())
    )
...
class SalesOrderShipment(
    ...
    def complete_shipment(self, user, **kwargs):
        ...
        self.shipment_date = kwargs.get(
            'shipment_date', Tracklet.helpers.current_date()
        )
        self.shipped_by = user
...
class ReturnOrder(TotalPriceMixin, Order):
    ...
    def receive_line_item(self, line, location, user, **kwargs):
        ...
        # Update the LineItem
        line.received_date = Tracklet.helpers.current_date()
        line.save()
