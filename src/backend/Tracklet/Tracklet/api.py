...
import Tracklet.config
import Tracklet.permissions
import Tracklet.version
from common.settings import get_global_setting
from Tracklet import helpers, ready
from Tracklet.auth_overrides import registration_enabled
from Tracklet.mixins import ListCreateAPI
...
class LicenseView(APIView):
    """Simple JSON endpoint for Tracklet license information."""
    ...
    def get(self, request, *args, **kwargs):
        """Return license information for the Tracklet server."""
        backend = Tracklet.config.get_base_dir().joinpath('Tracklet', 'licenses.txt')
        frontend = Tracklet.config.get_base_dir().joinpath(
...
class VersionView(APIView):
    """Simple JSON endpoint for Tracklet version information."""

    permission_classes = [Tracklet.permissions.IsAdminOrAdminScope]

    @extend_schema(responses={200: OpenApiResponse(response=VersionViewSerializer)})
    def get(self, request, *args, **kwargs):
        """Return version information for the Tracklet server."""
        return JsonResponse({
            'dev': Tracklet.version.isInvenTreeDevelopmentVersion(),
            'up_to_date': Tracklet.version.isInvenTreeUpToDate(),
            'version': {
                'server': Tracklet.version.inventreeVersion(),
                'api': Tracklet.version.inventreeApiVersion(),
                'commit_hash': Tracklet.version.inventreeCommitHash(),
                'commit_date': Tracklet.version.inventreeCommitDate(),
                'commit_branch': Tracklet.version.inventreeBranch(),
                'python': Tracklet.version.inventreePythonVersion(),
                'django': Tracklet.version.inventreeDjangoVersion(),
            },
            'links': {
                'doc': Tracklet.version.inventreeDocUrl(),
                'code': Tracklet.version.inventreeGithubUrl(),
                'app': Tracklet.version.inventreeAppUrl(),
                'bug': f'{Tracklet.version.inventreeGithubUrl()}issues',
            },
        })
...
class VersionTextView(ListAPI):
    """Simple JSON endpoint for Tracklet version text."""
    ...
    def list(self, request, *args, **kwargs):
        """Return version text information for the Tracklet server."""
...
class InfoApiSerializer(serializers.Serializer):
    """Tracklet server information - some information might be blanked if called without elevated credentials."""
...
class InfoView(APIView):
    """JSON endpoint for Tracklet server information.

    Use to confirm that the server is running, etc.
    """

    permission_classes = [Tracklet.permissions.AllowAnyOrReadScope]
    ...
    @extend_schema(
        responses={
            200: OpenApiResponse(
                response=InfoApiSerializer, description='Tracklet server information'
            )
        }
    )
    def get(self, request, *args, **kwargs):
        ...
        data = {
            'server': 'Tracklet',
            'id': Tracklet.version.inventree_identifier(),
            'version': Tracklet.version.inventreeVersion(),
            'instance': Tracklet.version.inventreeInstanceName(),
            'apiVersion': Tracklet.version.inventreeApiVersion(),
            ...
            'database': Tracklet.version.inventreeDatabase() if is_staff else None,
            'platform': Tracklet.version.inventreePlatform() if is_staff else None,
            'installer': Tracklet.config.inventreeInstaller() if is_staff else None,
            'target': Tracklet.version.inventreeTarget() if is_staff else None,
    ...
    def check_auth_header(self, request):
        ...
        from Tracklet.middleware import get_token_from_request
...
class GenericMetadataView(RetrieveUpdateAPI):
    """Metadata for specific instance; see the Tracklet documentation for more detail on how metadata works."""

    model = None  # Placeholder for the model class
    serializer_class = MetadataSerializer
    permission_classes = [Tracklet.permissions.ContentTypePermission]
