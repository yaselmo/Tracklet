...
from Tracklet.mixins import ListCreateAPI, OutputOptionsMixin
from Tracklet.unit_test import InvenTreeAPITestCase
from Tracklet.urls import backendpatterns
...
class SampleSerializer(
    ...
    field_b = Tracklet.serializers.enable_filter(
        Tracklet.serializers.FilterableSerializerMethodField(method_name='sample')
    )
    field_c = Tracklet.serializers.enable_filter(
        Tracklet.serializers.FilterableSerializerMethodField(method_name='sample'),
        True,
        filter_name='crazy_name',
    )
    field_d = Tracklet.serializers.enable_filter(
        Tracklet.serializers.FilterableSerializerMethodField(method_name='sample'),
        True,
        filter_name='crazy_name',
    )
    field_e = Tracklet.serializers.enable_filter(
        Tracklet.serializers.FilterableSerializerMethodField(method_name='sample'),
        filter_name='field_e',
...
class FilteredSerializers(InvenTreeAPITestCase):
    ...
    def test_failiure_enable_filter(self):
        ...
        # Allowed usage
        field_b = Tracklet.serializers.enable_filter(  # noqa: F841
            Tracklet.serializers.FilterableSerializerMethodField(method_name='sample')
        )

        # Disallowed usage
        with self.assertRaises(Exception) as cm:
            field_a = Tracklet.serializers.enable_filter(  # noqa: F841
    ...
    def test_failure_OutputOptionsMixin(self):
        ...
        class BadSerializer(Tracklet.serializers.InvenTreeModelSerializer):
            """Sample serializer."""
