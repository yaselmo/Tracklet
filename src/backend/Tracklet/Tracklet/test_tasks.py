...
from Tracklet.unit_test import PluginRegistryMixin
...
class ScheduledTaskTests(TestCase):
    ...
    def test_add_task(self):
        ...
        task = 'Tracklet.tasks.heartbeat'

        self.assertEqual(self.get_tasks(task).count(), 0)

        Tracklet.tasks.schedule_task(task, schedule_type=Schedule.MINUTES, minutes=10)
        ...
        Tracklet.tasks.schedule_task(task, schedule_type=Schedule.MINUTES, minutes=5)
        self.assertEqual(self.get_tasks(task).count(), 1)
...
class InvenTreeTaskTests(PluginRegistryMixin, TestCase):
    ...
    def test_offloading(self):
        ...
        Tracklet.tasks.offload_task(get_result)

        # Run with string ref
        Tracklet.tasks.offload_task('Tracklet.test_tasks.get_result')

        # Error runs
        # Malformed taskname
        with self.assertWarnsMessage(
            UserWarning, "WARNING: 'Tracklet' not started - Malformed function path"
        ):
            Tracklet.tasks.offload_task('Tracklet')

        # Non existent app
        with self.assertWarnsMessage(
            UserWarning,
            "WARNING: 'TrackletABC.test_tasks.doesnotmatter' not started - No module named 'TrackletABC.test_tasks'",
        ):
            Tracklet.tasks.offload_task('TrackletABC.test_tasks.doesnotmatter')

        # Non existent function
        with self.assertWarnsMessage(
            UserWarning,
            "WARNING: 'Tracklet.test_tasks.doesnotexist' not started - No function named 'doesnotexist'",
        ):
            Tracklet.tasks.offload_task('Tracklet.test_tasks.doesnotexist')

    def test_task_heartbeat(self):
        """Test the task heartbeat."""
        Tracklet.tasks.offload_task(Tracklet.tasks.heartbeat)
    ...
    def test_task_delete_successful_tasks(self):
        ...
        Tracklet.tasks.offload_task(Tracklet.tasks.delete_successful_tasks)
    ...
    def test_task_delete_old_error_logs(self):
        ...
        # Run action
        Tracklet.tasks.offload_task(Tracklet.tasks.delete_old_error_logs)
    ...
    def test_task_check_for_updates(self):
        ...
        # Get new version
        Tracklet.tasks.offload_task(Tracklet.tasks.check_for_updates)
    ...
    def test_task_check_for_migrations(self):
        ...
        # Update disabled
        Tracklet.tasks.check_for_migrations()

        # Update enabled - no migrations
        os.environ['INVENTREE_AUTO_UPDATE'] = 'True'
        Tracklet.tasks.check_for_migrations()

        # List all current migrations for the Tracklet app
        ...
        # Create migration
        self.assertEqual(len(Tracklet.tasks.get_migration_plan()), 0)
        call_command('makemigrations', ['Tracklet', '--empty'], interactive=False)
        self.assertEqual(len(Tracklet.tasks.get_migration_plan()), 1)
        ...
        # Cleanup
        try:
            migration_name = Tracklet.tasks.get_migration_plan()[0][0].name + '.py'
            migration_path = (
                settings.BASE_DIR / 'Tracklet' / 'migrations' / migration_name
            )
    ...
    def test_failed_task_notification(self):
        ...
        test_data = {
            'name': 'failed_task',
            'func': 'Tracklet.tasks.failed_task',
            'group': 'test',
            'success': False,
        ...
        self.assertEqual(
            msg.message,
            "Background worker task 'Tracklet.tasks.failed_task' failed after 10 attempts",
        )
    ...
    def test_delete_old_emails(self):
        ...
        Tracklet.tasks.offload_task(Tracklet.tasks.delete_old_emails, force_sync=True)
