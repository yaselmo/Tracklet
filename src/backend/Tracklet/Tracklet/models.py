...
class ReferenceIndexingMixin(models.Model):
    ...
    def get_next_reference(cls):
        ...
        try:
            reference = Tracklet.format.extract_named_group(
        ...
        incremented = Tracklet.helpers.increment(reference)
            ...
            try:
                ...
                if ref in attempts:
                    ...
                    if cls.objects.filter(reference=ref).exists():
                        ...
                        ctx['ref'] = Tracklet.helpers.increment(ctx['ref'])
                    else:
    ...
    def validate_reference_pattern(cls, pattern):
        ...
        try:
            info = Tracklet.format.parse_format_string(pattern)
    ...
    def validate_reference_field(cls, value):
        ...
        if not Tracklet.format.validate_string(value, pattern):
            raise ValidationError(
    ...
    def rebuild_reference_field(cls, reference, validate=False):
        ...
        reference_int = Tracklet.helpers.extract_int(reference)
...
class InvenTreeTree(ContentTypeMixin, MPTTModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if len(trees) > 0:
            ...
            try:
                ...
                Tracklet.sentry.report_exception(e)
                Tracklet.exceptions.log_error(f'{self.__class__.__name__}.save')
    ...
    def partial_rebuild(self, tree_id: int) -> bool:
        ...
        try:
            ...
            Tracklet.exceptions.log_error(f'{self.__class__.__name__}.partial_rebuild')
            logger.exception(
...
class PathStringMixin(models.Model):
    ...
    def construct_pathstring(self, refresh: bool = False) -> str:
        ...
        return Tracklet.helpers.constructPathString([
...
class InvenTreeNotesMixin(models.Model):
    ...
    notes = Tracklet.fields.InvenTreeNotesField(
...
class InvenTreeBarcodeMixin(models.Model):
    ...
    def assign_barcode(
        ...
        if barcode_hash is None and barcode_data is not None:
            barcode_hash = Tracklet.helpers.hash_barcode(barcode_data)
...
def after_failed_task(sender, instance: Task, created: bool, **kwargs):
    ...
    if not instance.success and n >= max_attempts:
        try:
            url = Tracklet.helpers_model.construct_absolute_url(
                reverse(
...
class InvenTreeImageMixin(models.Model):
    ...
    def get_image_url(self):
        ...
        if self.image:
            return Tracklet.helpers.getMediaUrl(self.image)
        return Tracklet.helpers.getBlankImage()
