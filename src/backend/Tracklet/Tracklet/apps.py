...
from Tracklet.config import get_setting
...
class InvenTreeConfig(AppConfig):
    ...
    def ready(self):
        ...
        if not Tracklet.ready.isPluginRegistryLoaded():
            return

        # Skip if not in worker or main thread
        if (
            not Tracklet.ready.isInMainThread()
            and not Tracklet.ready.isInWorkerThread()
        ):
            return
        ...
        if Tracklet.ready.canAppAccessDatabase() or settings.TESTING_ENV:
            ...
            if not Tracklet.ready.isInTestMode():  # pragma: no cover
                # Update exchange rates
                Tracklet.tasks.offload_task(Tracklet.tasks.update_exchange_rates)
                # Let the background worker check for migrations
                Tracklet.tasks.offload_task(Tracklet.tasks.check_for_migrations)
    ...
    def remove_obsolete_tasks(self):
        ...
        obsolete = [
            'Tracklet.tasks.delete_expired_sessions',
    ...
    def start_background_tasks(self):
        """Start all background tests for Tracklet."""
        ...
        tasks = Tracklet.tasks.tasks.task_list
        ...
        try:
            if django_q.models.OrmQ.objects.count() == 0:
                Tracklet.tasks.offload_task(
                    Tracklet.tasks.heartbeat, force_async=True, group='heartbeat'
                )
        except AppRegistryNotReady:  # pragma: no cover
            pass
        except Exception:
    ...
    def update_site_url(self):
        ...
        """
        if not Tracklet.ready.canAppAccessDatabase():
            return
    ...
    def load_unit_registry(self):
        ...
        Tracklet.conversion.get_unit_registry()
    ...
    def ensure_migrations_done(self=None):
        ...
        if not Tracklet.tasks.check_for_migrations():
            # Detect if this an empty database - if so, start with a fresh migration
            if (
                settings.DOCKER
                and not Tracklet.ready.isInTestMode()
                and not Tracklet.ready.isRunningMigrations()
                and Tracklet.tasks.get_migration_count() == 0
            ):
                logger.warning(
                    'INVE-W8: Empty database detected - trying to run migrations'
                )
                Tracklet.tasks.check_for_migrations(force_run=True)
            else:
