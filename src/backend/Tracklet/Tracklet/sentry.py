...
def default_sentry_dsn():
    """Return the default Sentry.io DSN for Tracklet."""
...
def init_sentry(dsn, sample_rate, tags):  # pragma: no cover
    ...
    sentry_sdk.init(
        ...
        release=Tracklet.version.INVENTREE_SW_VERSION,
        environment='development'
        if Tracklet.version.isInvenTreeDevelopmentVersion()
        else 'production',
    )
    ...
    sentry_sdk.set_tag('api', Tracklet.version.inventreeApiVersion())
    sentry_sdk.set_tag('platform', Tracklet.version.inventreePlatform())
    sentry_sdk.set_tag('git_branch', Tracklet.version.inventreeBranch())
    sentry_sdk.set_tag('git_commit', Tracklet.version.inventreeCommitHash())
    sentry_sdk.set_tag('git_date', Tracklet.version.inventreeCommitDate())
