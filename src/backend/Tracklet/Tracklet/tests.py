...
import Tracklet.conversion
import Tracklet.format
import Tracklet.helpers
import Tracklet.helpers_model
...
from Tracklet.helpers_mixin import ClassProviderMixin, ClassValidationMixin
from Tracklet.sanitizer import sanitize_svg
from Tracklet.unit_test import InvenTreeTestCase, in_env_context
...
class ConversionTest(TestCase):
    ...
    def test_prefixes(self):
        ...
        for val, expected in tests.items():
            q = Tracklet.conversion.convert_physical_value(val, 'm')
    ...
    def test_engineering_units(self):
        ...
        for val, expected in tests:
            self.assertEqual(
                Tracklet.conversion.from_engineering_notation(val), expected
            )
        ...
        for val, expected in tests:
            output = Tracklet.conversion.convert_physical_value(
    ...
    def test_scientific_notation(self):
        ...
        for val, expected in tests:
            output = Tracklet.conversion.convert_physical_value(val, strip_units=True)
    ...
    def test_base_units(self):
        ...
        for val, expected in tests.items():
            q = Tracklet.conversion.convert_physical_value(val, 'W')
            self.assertAlmostEqual(q, expected, places=2)
            q = Tracklet.conversion.convert_physical_value(val, 'W', strip_units=False)
    ...
    def test_dimensionless_units(self):
        ...
        for val, expected in tests.items():
            ...
            q = Tracklet.conversion.convert_physical_value(val, strip_units=False)
            self.assertAlmostEqual(float(q.magnitude), expected, 3)

            # Convert, and strip units
            q = Tracklet.conversion.convert_physical_value(val)
            self.assertAlmostEqual(q, expected, 3)
    ...
    def test_invalid_units(self):
        ...
        for val, unit in tests.items():
            with self.assertRaises(ValidationError):
                Tracklet.conversion.convert_physical_value(val, unit)
    ...
    def test_invalid_values(self):
        ...
        for val in inputs:
            ...
            with self.assertRaises(ValidationError):
                Tracklet.conversion.convert_physical_value(val, 'meter')

            # Test dimensionless
            with self.assertRaises(ValidationError):
                Tracklet.conversion.convert_physical_value(val)

    def test_custom_units(self):
        """Tests for custom unit conversion."""
        # Start with an empty set of units
        CustomUnit.objects.all().delete()
        Tracklet.conversion.reload_unit_registry()

        # Ensure that the custom unit does *not* exist to start with
        reg = Tracklet.conversion.get_unit_registry()
        ...
        for val, expected in tests.items():
            # Convert, and leave units
            q = Tracklet.conversion.convert_physical_value(
                val, 'henry / km', strip_units=False
            )
            self.assertAlmostEqual(float(q.magnitude), expected, 2)

            # Convert and strip units
            q = Tracklet.conversion.convert_physical_value(val, 'henry / km')
...
class FormatTest(TestCase):
    ...
    def test_parse(self):
        ...
        info = Tracklet.format.parse_format_string(fmt)
        ...
        for fmt in ['PO-{{xyz}', 'PO-{xyz}}', 'PO-{xyz}-{']:
            with self.assertRaises(ValueError):
                Tracklet.format.parse_format_string(fmt)
    ...
    def test_create_regex(self):
        ...
        for fmt, reg in tests.items():
            self.assertEqual(Tracklet.format.construct_format_regex(fmt), reg)
    ...
    def test_validate_format(self):
        ...
        for value, pattern in {
            ...
            self.assertTrue(Tracklet.format.validate_string(value, pattern))
        ...
        for value, pattern in {
            ...
            self.assertFalse(Tracklet.format.validate_string(value, pattern))
    ...
    def test_extract_value(self):
        ...
        for k, v in tests.items():
            self.assertEqual(Tracklet.format.extract_named_group('ref', v, fmt), k)
        ...
        for v in tests.values():
            with self.assertRaises(ValueError):
                Tracklet.format.extract_named_group('ref', v, fmt)
        ...
        for k, v in data.items():
            self.assertEqual(Tracklet.format.extract_named_group(k, val, fmt), v)

        # Test for error conditions

        # Raises a ValueError as the format string is bad
        with self.assertRaises(ValueError):
            Tracklet.format.extract_named_group('test', 'PO-1234-5', 'PO-{test}-{')

        # Raises a NameError as the named group does not exist in the format string
        with self.assertRaises(NameError):
            Tracklet.format.extract_named_group('missing', 'PO-12345', 'PO-{test}')

        # Raises a ValueError as the value does not match the format string
        with self.assertRaises(ValueError):
            Tracklet.format.extract_named_group('test', 'PO-1234', 'PO-{test}-1234')

        with self.assertRaises(ValueError):
            Tracklet.format.extract_named_group('test', 'PO-ABC-xyz', 'PO-###-{test}')
    ...
    def test_currency_formatting(self):
        ...
        with self.settings(LANGUAGE_CODE='en-us'):
            for value, decimal_places, include_symbol, expected_result in test_data:
                result = Tracklet.format.format_money(
...
class TestHelpers(TestCase):
    ...
    def test_absolute_url(self):
        ...
        for url, expected in tests.items():
            ...
            self.assertEqual(
                Tracklet.helpers_model.construct_absolute_url(url, base_url=base),
                expected,
            )

            # Test without supplied base URL
            self.assertEqual(
                Tracklet.helpers_model.construct_absolute_url(url), expected
            )
    ...
    def test_logo_image(self):
        ...
        self.assertEqual(logo, '/static/img/tracklet.png')

        logo = helpers.getLogoImage(as_file=True)
        self.assertEqual(logo, f'file://{settings.STATIC_ROOT}/img/tracklet.png')
    ...
    def test_download_image(self):
        ...
        for url in ['blog', 'htp://test.com/?', 'google', '\\invalid-url']:
            with self.assertRaises(django_exceptions.ValidationError):
                Tracklet.helpers_model.download_image_from_url(url)

        # We mock the network request below, so no actual download is performed
        large_img = 'https://example.com/paper_splash_large.jpg'

        # Prepare a tiny valid PNG for the "successful download" case
        buf = io.BytesIO()
        Image.new('RGB', (10, 10), color='white').save(buf, format='PNG')
        ok_bytes = buf.getvalue()

        class DummyResponse:
            def __init__(self, content_length: int, content_bytes: bytes = b''):
        ...
        def mocked_get(url, *args, **kwargs):
            ...
            # First request: too large for 1MB limit
            if call_count['n'] == 1:
                return DummyResponse(content_length=2 * 1024 * 1024)

            # Second request: small valid image
            return DummyResponse(content_length=len(ok_bytes), content_bytes=ok_bytes)
        ...
        with mock.patch('Tracklet.helpers_model.requests.get', side_effect=mocked_get):
            with self.assertRaises(ValueError):
                Tracklet.helpers_model.download_image_from_url(large_img, timeout=10)
    ...
    def test_model_mixin(self):
        ...
        from Tracklet.models import InvenTreeBarcodeMixin

        models = Tracklet.helpers_model.getModelsWithMixin(InvenTreeBarcodeMixin)
...
class TestTimeFormat(TestCase):
    ...
    def test_tz_utc(self):
        """Check UTC timezone."""
        self.assertEqual(Tracklet.helpers.server_timezone(), 'UTC')

    @override_settings(TIME_ZONE='Europe/London')
    def test_tz_london(self):
        """Check London timezone."""
        self.assertEqual(Tracklet.helpers.server_timezone(), 'Europe/London')
    ...
    def test_to_local_time(self):
        ...
        for tz, expected in tests:
            local_time = Tracklet.helpers.to_local_time(source_time, tz)
...
class CurrencyTests(TestCase):
    ...
    def test_rates(self):
        ...
        for _ in range(10):
            Tracklet.tasks.update_exchange_rates()
...
class TestSettings(InvenTreeTestCase):
    ...
    def run_reload(self, envs=None):
        """Helper function to reload Tracklet."""
...
class TestInstanceName(InvenTreeTestCase):
    ...
    def test_instance_name(self):
        ...
        # default setting
        self.assertEqual(version.inventreeInstanceTitle(), 'Tracklet')
...
class TestOffloadTask(InvenTreeTestCase):
    ...
    def test_daily_holdoff(self):
        ...
        with self.assertLogs(logger='inventree', level='INFO') as cm:
            ...
            result = Tracklet.tasks.check_daily_holdoff('some_task', 0)
            self.assertFalse(result)
            self.assertIn('Specified interval', str(cm.output))

        with self.assertLogs(logger='inventree', level='INFO') as cm:
            # First call should run without issue
            result = Tracklet.tasks.check_daily_holdoff('dummy_task')
        ...
        result = Tracklet.tasks.check_daily_holdoff('dummy_task', 5)
        self.assertTrue(result)
        ...
        with self.assertLogs(logger='inventree', level='INFO') as cm:
            result = Tracklet.tasks.check_daily_holdoff('dummy_task', 7)
            self.assertFalse(result)
            self.assertIn('Last successful run for', str(cm.output))

            result = Tracklet.tasks.check_daily_holdoff('dummy_task', 2)
            self.assertTrue(result)
...
class BarcodeMixinTest(InvenTreeTestCase):
    ...
    def test_barcode_hash(self):
        ...
        for barcode, expected in hashing_tests.items():
            self.assertEqual(Tracklet.helpers.hash_barcode(barcode), expected)
...
class SanitizerTest(TestCase):
    ...
    def test_svg_sanitizer(self):
        ...
        <path id="path1" d="m -151.78571,359.62883 v 112.76373 l 97.068507,-56.04253 V 303.14815 Z" style="fill:#4e7574;"></path>
        </svg>"""
