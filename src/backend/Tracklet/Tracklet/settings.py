"""Django settings for Tracklet project.
...
import Tracklet.backup
from Tracklet.cache import get_cache_config, is_global_cache_enabled
from Tracklet.config import (
    get_boolean_setting,
    get_custom_file,
    get_oidc_private_key,
    get_setting,
)
from Tracklet.ready import isInMainThread
from Tracklet.sentry import default_sentry_dsn, init_sentry
from Tracklet.version import checkMinPythonVersion, inventreeCommitHash
...
# Default news feed is disabled for Tracklet.
# If you want in-app news, set INVENTREE_NEWS_URL to a valid feed URL.
INVENTREE_BASE_URL = 'https://github.com/yaselmo/Tracklet'
INVENTREE_NEWS_URL = ''
...
DBBACKUP_DATE_FORMAT = Tracklet.backup.backup_date_format()
DBBACKUP_FILENAME_TEMPLATE = Tracklet.backup.backup_filename_template()
DBBACKUP_MEDIA_FILENAME_TEMPLATE = Tracklet.backup.backup_media_filename_template()

DBBACKUP_GPG_RECIPIENT = Tracklet.backup.backup_gpg_recipient()

DBBACKUP_SEND_EMAIL = Tracklet.backup.backup_email_on_error()
DBBACKUP_EMAIL_SUBJECT_PREFIX = Tracklet.backup.backup_email_prefix()

DBBACKUP_CONNECTORS = {'default': Tracklet.backup.get_backup_connector_options()}

# Data storage options
DBBACKUP_STORAGE_CONFIG = {
    'BACKEND': Tracklet.backup.get_backup_storage_backend(),
    'OPTIONS': Tracklet.backup.get_backup_storage_options(),
}
...
INSTALLED_APPS = [
    ...
    'tracklet_events.apps.EventsConfig',
    ...
    'web',
    'generic',
    'Tracklet.apps.InvenTreeConfig',  # Tracklet app runs last
    # Core django modules
    'django.contrib.auth',
...
MIDDLEWARE = CONFIG.get(
    ...
    [
        ...
        'Tracklet.middleware.InvenTreeRemoteUserMiddleware',  # Remote / proxy auth
        ...
        'Tracklet.middleware.InvenTreeExceptionProcessor',  # Error reporting
        'Tracklet.middleware.InvenTreeRequestCacheMiddleware',  # Request caching
        'Tracklet.middleware.InvenTreeHostSettingsMiddleware',  # Ensuring correct hosting/security settings
...
# Tracklet URL configuration
ROOT_URLCONF = 'Tracklet.urls'
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            ...
            'loaders': [
                (
                    'Tracklet.template.InvenTreeTemplateLoader',
                    [
...
REST_FRAMEWORK = {
    'EXCEPTION_HANDLER': 'Tracklet.exceptions.exception_handler',
    ...
    'DEFAULT_PERMISSION_CLASSES': [
        ...
        'Tracklet.permissions.ModelPermission',
        'Tracklet.permissions.RolePermission',
        'Tracklet.permissions.InvenTreeTokenMatchesOASRequirements',
    ],
    'DEFAULT_SCHEMA_CLASS': 'Tracklet.schema.ExtendedAutoSchema',
    'DEFAULT_METADATA_CLASS': 'Tracklet.metadata.InvenTreeMetadata',
...
WSGI_APPLICATION = 'Tracklet.wsgi.application'
...
EXCHANGE_BACKEND = 'Tracklet.exchange.InvenTreeExchange'

# region email
# Email configuration options
EMAIL_BACKEND = 'Tracklet.backends.InvenTreeMailLoggingBackend'
...
EMAIL_SUBJECT_PREFIX = get_setting(
    'INVENTREE_EMAIL_PREFIX', 'email.prefix', '[Tracklet] '
)
...
if SITE_URL:
    ...
    try:
        ...
        msg = f"Invalid SITE_URL value: '{SITE_URL}'. Tracklet server cannot start."
        logger.error(msg)
        print(msg)
        sys.exit(-1)
...
ACCOUNT_FORMS = {
    'login': 'Tracklet.auth_overrides.CustomLoginForm',
    'signup': 'Tracklet.auth_overrides.CustomSignupForm',
...
SOCIALACCOUNT_ADAPTER = 'Tracklet.auth_overrides.CustomSocialAccountAdapter'
ACCOUNT_ADAPTER = 'Tracklet.auth_overrides.CustomAccountAdapter'
HEADLESS_ADAPTER = 'Tracklet.auth_overrides.CustomHeadlessAdapter'
ACCOUNT_LOGOUT_ON_PASSWORD_CHANGE = True
...
if not MFA_ENABLED:
    MIDDLEWARE.remove('Tracklet.middleware.Check2FAMiddleware')
...
MAINTENANCE_MODE_STATE_BACKEND = 'Tracklet.backends.InvenTreeMaintenanceModeBackend'
...
if DEBUG:
    logger.info('Tracklet running with DEBUG enabled')
