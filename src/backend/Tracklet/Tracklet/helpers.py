...
def getLogoImage(as_file=False, custom=True):
    """Return the Tracklet logo image, or a custom logo if available."""
    ...
    if as_file:
        path = settings.STATIC_ROOT.joinpath('img/tracklet.png')
        return f'file://{path}'
    return getStaticUrl('img/tracklet.png')


def getSplashScreen(custom=True):
    """Return the Tracklet splash screen, or a custom splash if available."""
    static_storage = StaticFilesStorage()
    ...
    return static_storage.url('img/tracklet_splash.jpg')
