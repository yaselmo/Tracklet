...
from Tracklet.config import CONFIG_LOOKUPS
from Tracklet.filters import (
    ORDER_FILTER,
    SEARCH_ORDER_FILTER,
    SEARCH_ORDER_FILTER_ALIAS,
)
from Tracklet.helpers import inheritors, str2bool
from Tracklet.helpers_email import send_email
...
from Tracklet.permissions import (
    AllowAnyOrReadScope,
...
class CurrencyRefreshView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        from Tracklet.tasks import update_exchange_rates
...
class BackgroundTaskOverview(APIView):
    ...
    def get(self, request, fmt=None):
        ...
        import Tracklet.status

        serializer = common.serializers.TaskOverviewSerializer({
            'is_running': Tracklet.status.is_worker_running(),
...
class TestEmail(CreateAPI):
    ...
    def perform_create(self, serializer):
        ...
        delivered, reason = send_email(
            subject='Test email from Tracklet',
            body='This is a test email from Tracklet.',
...
class HealthCheckStatusSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        help_text='Health status of the Tracklet server',
        choices=['ok', 'loading'],
        read_only=True,
        default='ok',
    )
...
class HealthCheckView(APIView):
    """Simple JSON endpoint for Tracklet health check.

    Intended to be used by external services to confirm that the Tracklet server is running.
    """
    ...
    @extend_schema(
        responses={
            200: OpenApiResponse(
                ...
                description='Tracklet server health status',
            )
        }
    )
    def get(self, request, *args, **kwargs):
        ...
        """
        status = (
            Tracklet.ready.isPluginRegistryLoaded()
