...
def overlapping_project_reservation_filter(
    ...
    - If the current project has no dates at all, do not block on project overlap.
    - Null dates on *other* projects are treated as open-ended boundaries to remain
      conservative when their active window is incomplete.
    """
    ...
    if project.start_date is None and project.end_date is None:
        # Without any current-project dates, we cannot evaluate overlap safely.
        # Return an always-false filter here so we do not incorrectly hide every
        # project-reserved item from the selector or over-restrict validation.
        return overlap & Q(**{f'{prefix}pk__isnull': True})
