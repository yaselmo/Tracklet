...
export function extractErrorMessage({
  ...
  if (error_data) {
    if (typeof error_data === 'string') {
      ...
      const fieldError = error_data[field ?? 'error'];
      const detail = error_data['detail'];
      const nonFieldErrors = error_data['non_field_errors'];
      const errors = error_data['errors'];

      if (typeof fieldError === 'string') {
        message = fieldError;
      } else if (typeof detail === 'string') {
        message = detail;
      } else if (Array.isArray(nonFieldErrors)) {
        ...
        message = errors
          .map((entry: any) =>
            typeof entry === 'string'
              ? entry
              : entry?.message || entry?.detail || JSON.stringify(entry)
          )
          .join(', ');
      }
    }
