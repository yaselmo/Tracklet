...
export function ApiForm({
  ...
  const submitForm: SubmitHandler<FieldValues> = async (data) => {
    ...
    return api({
      ...
      .catch((error) => {
        if (import.meta.env.DEV) {
          console.error('[ApiForm] submit error', {
            method,
            url,
            payload: jsonData,
            payloadJson: jsonPayloadString,
            responseStatus: error.response?.status ?? error.status ?? null,
        ...
        if (error.response) {
          switch (error.response.status) {
            case 400:
              ...
              break;
            default:
              // Surface the backend response body for non-validation errors such as 403s.
              setNonFieldErrors([
                extractErrorMessage({
                  error,
                  defaultMessage: t`Request failed`
                })
              ]);
              props.onFormError?.(error, form);
              break;
