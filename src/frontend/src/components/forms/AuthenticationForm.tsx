...
export function AuthenticationForm() {
  ...
  async function handleCreateSuperuser() {
    ...
    setIsLaunchingSuperuser(true);

    try {
      const result = await window.TRACKLET_ELECTRON?.openCreateSuperuser?.();
      ...
      if (result?.ok) {
        showNotification({
          title: t`Superuser helper launched`,
          message: t`A PowerShell window opened to run Django createsuperuser for your Tracklet backend.`,
          color: 'green'
        });
        return;
      }
      ...
      showNotification({
        title: t`Unable to launch superuser helper`,
        message:
          result?.error ||
          t`Tracklet Desktop could not start the backend superuser helper.`,
        color: 'red'
      });
    } catch (error) {
      showNotification({
      ...
      {canCreateSuperuser || canCreateBackup ? (
        <Alert color='green' title={t`Desktop setup helper`} mb='md'>
          <Stack gap='xs'>
            <Text size='sm'>
              <Trans>
                Need the first admin account after installation? Launch the
                Tracklet superuser helper and point it at your existing backend
                folder.
              </Trans>
            </Text>
            {canCreateSuperuser ? (
